...
func TestParseFullState(t *testing.T) {
	line := "STATE:seq=0001,clock=16:42:03,temp=1D6,light=038,nav=3FF,ext0=000,ext1=001,hall=1,vib=0,k1=1,k2=0,k3=0,navkey=0,motor=free,beep=busy,led=FF,display=clock"
	s, ok := ParseFullState(line)
	if !ok {
		t.Fatal("expected valid STATE")
	}
	if s.Clock != "16:42:03" || s.Temp != 0x1D6 || s.Light != 0x038 || s.Hall != 1 || s.Key1 != 1 || s.Motor != "free" || s.Beep != "busy" || s.LED != 255 || s.Display != "clock" {
		t.Fatalf("state=%+v", s)
	}
}
